    ...
    <div className="min-h-screen bg-ink text-bone p-8">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            <span className="text-pop text-xs font-bold tracking-widest uppercase">Trainer</span>
            <h1 className="text-2xl font-bold mt-1">Dashboard Trainer</h1>
            {user?.branch && (
              <p className="text-bone/40 text-sm mt-0.5">{user.branch.name}</p>
            )}
          </div>
          <button
        ...
        <p className="text-bone/30 text-sm mt-8 text-center">
          Modul trainer — akan dikembangkan di tahap berikutnya.
        </p>
      </div>
    </div>
  )
